...
const OBJLoader = (function () {
  ...
  let objectPattern = /^[og]\s*(.+)?/
  // mtllib file_reference
  let materialLibraryPattern = /^mtllib /
  // usemtl material_name
  let materialUsePattern = /^usemtl /
  ...
  function ParserState () {
    let state = {
      ...
      addLineGeometry: function (vertices, uvs) {
        ...
        for (let uvi = 0, ll = uvs.length; uvi < ll; uvi++) {
    ...
    parse: function (text) {
      ...
      for (let i = 0, l = lines.length; i < l; i++) {
        ...
        if (lineFirstChar === 'v') {
          ...
          for (let jj = 1, jjl = faceVertices.length - 1; jj < jjl; jj++) {
            let v2 = faceVertices[ jj ]
            let v3 = faceVertices[ jj + 1 ]
