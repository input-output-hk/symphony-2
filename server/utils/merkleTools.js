...
module.exports.getMerklePositions = function (nTX) {
  nTX--
  nTX |= nTX >> 1
  nTX |= nTX >> 2
  nTX |= nTX >> 4
...
module.exports.drawMerkleCanvas = function (canvas, closestBlock, nTX, canvasSize, closestBlockOffsets) {
  let context = canvas.getContext('2d')

  const merkleData = module.exports.getMerklePositions(nTX)
  ...
  let offsetStack = []
  for (const key in closestBlockOffsets) {
    if (closestBlockOffsets.hasOwnProperty(key)) {
      offsetStack.push(closestBlockOffsets[key])
    }
  }
  ...
  context.fillText('BLOCK #' + closestBlock.height + '  HASH: ' + closestBlock.hash, -495, -495)
  context.scale(-1, 1)
